...
class LabelPredictor:
    def __init__(
        self, model: str, keep_nei: bool, threshold: float, device: torch.device
    ):
        self.device = device
    ...
    def __call__(self, claim: str, retrievals: List[dict]):
        results = []
        with torch.no_grad():
            for retrieval in tqdm(retrievals, disable=True):
                if not retrieval["evidence"]:
                    continue
                sentences = [retrieval["abstract"][i] for i in retrieval["evidence"]]
                encoded_dict = self.tokenizer.batch_encode_plus(
                    [(" ".join(sentences), claim)], return_tensors="pt"
                )
                encoded_dict = {
                ...
                keep = (label_score > self.threshold) and (
                    self.keep_nei or (label_index != 1)
                )
                if keep:
